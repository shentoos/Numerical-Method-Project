...
% See also: GUIDE, GUIDATA, GUIHANDLES
	...
	% Last Modified by GUIDE v2.5 13-Nov-2013 23:51:26
...
% varargin   command line arguments to chp3_menu (see VARARGIN)
	...
	set(handles.points_scrl_pnl,'Units', 'pixels', 'ScrollMode', 'verticle', ...
		'Position', [105 415 195 75], 'ScrollArea', [0 0 195 75]);
	...
	set(handles.nbd_rdbtn, 'Userdata', ...
		struct('type', 'interpolation', 'equally_spaced', true, ...
		'algorithm', @NB));
	set(handles.nfcd_rdbtn, 'Userdata', ...
		struct('type', 'interpolation', 'equally_spaced', true, ...
		'algorithm', @newton_forward_central_differences));
	set(handles.nbcd_rdbtn, 'Userdata', ...
		struct('type', 'interpolation', 'equally_spaced', true, ...
		'algorithm', @newton_backward_central_differences));
	...
	set(handles.curve5, 'Userdata', struct('type', 'fitting', ...
		'algorithm', @X1));
	set(handles.curve6, 'Userdata', struct('type', 'fitting', ...
		'algorithm', @X2));
	set(handles.curve7, 'Userdata', struct('type', 'fitting', ...
		'algorithm', @X3));
	set(handles.rms_rdbtn, 'Userdata', struct('type', 'fitting', ...
		'algorithm', @CF));
...
% handles    structure with handles and user data (see GUIDATA)
	...
	Y = getY(handles);
	fitted_func = algorithm(sort(X), Y);
...
function add_point_input(handles)
	...
	handles.(tag_x) = uicontrol('Style', 'edit', 'Tag', tag_x, 'String', '', ...
		'Units', 'pixels', 'Position', [10 25*point_count-20 80 20], ...
	...
	handles.(tag_y) = uicontrol('Style', 'edit', 'Tag', tag_y, 'String', '', ...
		'Units', 'pixels', 'Position', [95 25*point_count-20 80 20], ...
		'Parent', handles.points_scrl_pnl.handle, 'Callback', @(hObject, ...
		eventdata)point_input_Callback(hObject, eventdata, guidata(hObject)));
